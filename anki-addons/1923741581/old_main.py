...
def add_config_button(menu:QMenu):
    from .zoom_Config import SetAnkiRestartConfig
    from aqt.utils import qconnect
    mw.addonManager.setConfigAction(__name__, SetAnkiRestartConfig)
    addon_Action = QAction("Zoom for Anki25 (Fixed by Shige)", mw)
...
def setup_menu():
    ...
    mw.zoom_submenu = QMenu(_('&Zoom (Fixed by Shige)'), mw)
    mw.addon_view_menu.addMenu(mw.zoom_submenu)

    config = mw.addonManager.getConfig(__name__)

    zoom_in_shortcut = config.get("zoom_in_shortcut", "Ctrl+Shift++")
    zoom_out_shortcut = config.get("zoom_out_shortcut", "Ctrl+Shift+-")
    reset_shortcut = config.get("reset_shortcut", "Ctrl+Shift+^")
